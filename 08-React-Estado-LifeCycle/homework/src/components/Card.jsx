...
export default class Card extends Component {
  render() {
    return (
          ...
          <div className={this.props.min < 18 ? s.oneCard : s.oneCardHot}>
            <div className={s.close}>
              <button onClick={() => this.props.onClose(this.props.id)}>X</button> {/*recibo la func de Cards y le pongo el id de la card en la que estoy*/}
            </div>
            <div className={s.clima}>
              <h2 className={s.cityName}>{this.props.name}</h2>
              <div className={s.allclima}>
                <div className={s.divTemp}>
                  <h3>Min</h3>
                  <p className={s.temp}>{this.props.min}°</p>
                </div>
                <div className={s.divTemp}>
                  <h3>Max</h3>
                  <p className={s.temp}>{this.props.max}°</p>
                </div>
                <div className={s.divTemp}>
                  <h3>ST</h3>
                  <p className={s.temp}>{this.props.st}°</p>
                </div>
                <div className={s.imgTemp}>
                  <img className={s.imgI} src={`http://openweathermap.org/img/wn/${this.props.img}@2x.png`} alt='imagen del clima'/>
                </div>
              </div>
            </div>  
          </div> 
